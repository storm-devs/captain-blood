...
public:
	...
	void GetAllKeys(array<string> &returnArray)
	{
		for (dword i=0; i<dwSize; i++)
		{
			elem * pE = &pElements[i];
			for (dword j=0; j<pE->aSubElements.Size(); j++)
			{
				returnArray.Add(pE->aSubElements[j].pStr);
			}
		}
	}
	...
	bool Find(dword dwHashFullValue, const char * pStr, _Ty & _T) const
	{
		...
		for (dword i=0; i<pE.aSubElements.Size(); i++) 
		{
			...
			if (string::IsEqual(pSE.pStr , pStr)) 
			{
				_T = pSE._T;
				return true;
			}
	...
	__forceinline const _Ty & Get(const char * pStr) const
	{
		...
		for (dword i=0; i<pE.aSubElements.Size(); i++) 
		{
			...
			if (dwHashFullValue != pSE.dwHashFullValue) continue;
			if (string::IsEqual(pSE.pStr , pStr)) return pSE._T;
		}
		return _TBadFind;
	}

	__forceinline const _Ty * FindElement(const char * pStr) const
	{
		Assert(pStr && pElements);
		dword dwHashFullValue = string::HashNoCase(pStr);
		elem & pE = pElements[dwHashFullValue & dwMask];
		for (dword i=0; i<pE.aSubElements.Size(); i++) 
		{
			subelem & pSE = pE.aSubElements[i];
			if (dwHashFullValue != pSE.dwHashFullValue) continue;
			if (crt_stricmp(pSE.pStr , pStr) == 0) return &pSE._T;
		}
		return NULL;
	}
	...
	bool Del(const char * pStr)
	{
		...
		for (dword i=0; i<pE.aSubElements.Size(); i++) 
		{
			subelem & pSE = pE.aSubElements[i];
			if (dwHashFullValue != pSE.dwHashFullValue) continue;
			if (crt_stricmp(pSE.pStr , pStr) == 0) 
			{
				DELETE(pSE.pStr);
	...
	void ___sysDel(int iIndex, int iSubIndex)
	{
		elem & pE = pElements[iIndex];
		subelem & pSE = pE.aSubElements[iSubIndex];
		DELETE(pSE.pStr);
		pE.aSubElements.ExtractNoShift(iSubIndex);
	}
...
public:
	...
	public:
		...
		inline void DeleteCurrentItem()
		{
			...
			hTable.___sysDel(iIndex, iSubIndex);

			//что бы isDone срабатывал сразу..., т.к. итератор невалидный больше...
			iIndex = (long)hTable.dwSize;
		}
