...
DWORD OriginalAxisColors[3] = {0xFFFF0000, 0xFF00FF00, 0xFF0000FF};
DWORD AxisColors[3] = {0xFFFF0000, 0xFF00FF00, 0xFF0000FF};

#define X_AXIS_INDEX 0
#define Y_AXIS_INDEX 1
#define Z_AXIS_INDEX 2

TransformGizmo::TransformGizmo(GizmoType _type, GizmoMode _mode, const Matrix& _matTransform) : XAxis(_FL_, 256),
																																																YAxis(_FL_, 256),
																																																ZAxis(_FL_, 256)
{
	bEnabled = true;
...
void TransformGizmo::PushLine (GizmoAxis mode, const Vector& v1, const Vector& v2)
{
	switch(mode)
	{
		case GA_AXIS_X:
...
void TransformGizmo::CalcGizmoScale ()
{
...
void TransformGizmo::DrawMoveGizmo ()
{
	...
	if (mMode == TransformGizmo::GM_LOCAL)
	{
...
bool TransformGizmo::NeedDrawRotateGizmoLine (const Vector &v1, const Vector &v2, const Vector& vCamDir)
{
...
void TransformGizmo::DrawRotateGizmo ()
{
	if (!bMousePressed) CalcGizmoScale();
...
float TransformGizmo::GetDistanceBetweenPointAndLine (const Vector& line_start, const Vector& line_end, const Vector& point, bool bSegmentCollision)
{
...
/*
bool TransformGizmo::Intersection(float x, float y, const Vector &beg, const Vector &end)
{
	Vector p;

	p.x = 2.0f*x/GizmoviewPort.Width - 1.0f;
	p.y = 1.0f - 2.0f*y/GizmoviewPort.Height;

//	p.x *= matGizmoProjection.m[0][0];
//	p.y *= matGizmoProjection.m[1][1];

	p.z = 1.0f;

	p = pRS->GetProjection().MulVertexByInverse(p);
	p = pRS->GetView().MulVertexByInverse(p);

	pRS->DrawSphere(p,0.02f,-1);

	return true;
}
*/
bool TransformGizmo::LineLineIntersection (const Vector& line1_start, const Vector& line1_end, const Vector& line2_start, const Vector& line2_end, float Epsilon, Vector& intersection_point, bool bSegmentCollision)
{
...
void TransformGizmo::GetPickupRay (const Matrix& matProjection, const Matrix& matView, const RENDERVIEWPORT& ViewPort, float sX, float sY, Vector& rayStart, Vector& rayEnd)
{
	Vector v;
...
void TransformGizmo::SetView (const Matrix& mView)
{
	matGizmoView = mView;
...
void TransformGizmo::SetProjection  (const Matrix& mProjection)
{
	matGizmoProjection = mProjection;
}


void TransformGizmo::SetViewPort (const RENDERVIEWPORT& viewport)
{
	GizmoviewPort = viewport;
}

bool TransformGizmo::MouseMove(int x, int y)
{
	float fMouseX = (float)x;
	float fMouseY = (float)y;

	return HighlightAxis(fMouseX,fMouseY);
}

void TransformGizmo::MouseDrag(int x, int y)
{
	float fMouseX = (float)x;
	float fMouseY = (float)y;

	GizmoChangeTransform(fMouseX,fMouseY);

/*	Vector p;

	p.x = 2.0f*fMouseX/GizmoviewPort.Width - 1.0f;
	p.y = 1.0f - 2.0f*fMouseY/GizmoviewPort.Height;
...
//	p.y *= matGizmoProjection.m[1][1];
	...
	pRS->DrawSphere(p,0.05f,0xff00ff00);*/
}

bool TransformGizmo::HandleMessages(unsigned int uMsg, int LocalMouseX, int LocalMouseY)
{
...
bool TransformGizmo::HighlightAxis(float fMouseX, float fMouseY)
{
	Vector vPickupStart;
	Vector vPickupEnd;
	...
	//	Intersection(fMouseX,fMouseY,XAxis[i].vStart,XAxis[i].vEnd);
		if (bResult)
		{
...
bool TransformGizmo::GizmoChangeTransform (float fMouseX, float fMouseY)
{
	bool bRes = false;
...
bool TransformGizmo::GizmoChangeTransformMove (float fMouseX, float fMouseY)
{
...
bool TransformGizmo::GizmoChangeTransformRotate (float fMouseX, float fMouseY)
{
...
void TransformGizmo::CreateMouseLine(const Vector& v1, const Vector& v2)
{
...
const Matrix& TransformGizmo::GetTransform ()
{
	return mTransform;
}
...
void TransformGizmo::SetType (GizmoType _type)
{
	bMousePressed = false;
	mType = _type;
}
void TransformGizmo::SetMode (GizmoMode _mode)
{
	bMousePressed = false;
	mMode = _mode;
}

TransformGizmo::GizmoType TransformGizmo::GetType ()
{
	return mType;
}

TransformGizmo::GizmoMode TransformGizmo::GetMode ()
{
	return mMode;
}

void TransformGizmo::SetTransform (const Matrix& m)
{
	bMousePressed = false;
	mTransform = m;
}

bool TransformGizmo::IsActiveMode()
{
	return bMousePressed;
}

void TransformGizmo::Enable (bool _bEnabled)
{
	bMousePressed = false;
	bEnabled = _bEnabled;
}

bool TransformGizmo::IsEnabled ()
{
	return bEnabled;
}
