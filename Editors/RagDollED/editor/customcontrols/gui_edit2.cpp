...
// Сделать скролл...
GUIEdit2::GUIEdit2 (GUIControl* pParent, int Left, int Top, int Width, int Height) : GUIControl (pParent)
{
	PressedInside = false;
	Flat = false;
...
void GUIEdit2::Draw ()
{
	...
	if (nSelLength != 0)
	{
		...
		crt_strncpy (cTempBuffer, ARRSIZE(cTempBuffer), Text, ARRSIZE(cTempBuffer)-1);
...
void GUIEdit2::DrawEditBox(long nLeft, long nTop, long Width, long Height)
{
	if(!Flat)
	{
...
/*	Color col(0xffd4d0c8);
	...
	GUIHelper::DrawHorizLine(nLeft,Width,nTop,c);
	GUIHelper::DrawHorizLine(nLeft,Width,nTop + Height,c);

	GUIHelper::DrawVertLine(nTop,Height,nLeft,c);
	GUIHelper::DrawVertLine(nTop,Height,nLeft + Width,c);*/
...
bool GUIEdit2::ProcessMessages (GUIMessage message, DWORD lparam, DWORD hparam)
{
...
void GUIEdit2::OnMDown (int MouseKey, const GUIPoint& pt)
{
...
void GUIEdit2::OnMUp (int MouseKey, const GUIPoint& pt)
{
	t_OnMouseUp->Execute (this);
}


void GUIEdit2::OnMEnter ()
{
	if (ChangeCursor)
	{
...
void GUIEdit2::OnKPressed (int key, dword hparam)
{
	if (key <= 4) return;
	need_key_event = true;
	...
	if (hparam == 1)
	{
	...
	} else
		{
			...
			if (IsNumerical)
			{
				...
				if (!OnlyInteger&&key==46)
				{
					if (Text.FindSubStr(".")==-1)
					{
						Text.Insert (CursorPosition, szText);
						CursorPosition++;
					}
...
void GUIEdit2::CalcScrollOffset ()
{
...
void GUIEdit2::SelectText (int From, int Length)
{
	nSelStart = From;
	nSelLength = Length;
...
void GUIEdit2::MouseUp (int button, const GUIPoint& pt)
{
	PressedInside = false;
}

void GUIEdit2::MouseMove (int button, const GUIPoint& pt)
{
