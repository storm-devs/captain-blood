...
TCylinder::TCylinder(IRender* _pRS) : aPolygons(_FL_)
{
	pRS=_pRS;	
}
...
void TCylinder::Draw(float fRadius, float fHeight, dword dwColor, Matrix& WorldMat,
					const char * pTechniqueName)
{
	...
	pRS->DrawPrimitiveUP(id, PT_TRIANGLELIST, index, &aPolygons[0], sizeof(POLYGON_VERTEX));	
